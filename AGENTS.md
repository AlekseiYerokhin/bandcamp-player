...
- **One task = one branch.** A task stays on a single branch — do not
  preemptively split it into several branches. Branches of **up to ~3 commits
  are fine without asking**. If a task would need **more than 3 commits**,
  always **ask the user first** whether to keep going on the same branch or
  split into separate tasks (each on its own branch). Don't bundle unrelated
